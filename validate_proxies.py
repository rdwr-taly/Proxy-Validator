...
# SR3: machine-readable outcome for the ShowRunner report (main.py / report.py).
STATS_FILE_PATH = os.environ.get("VALIDATION_STATS", "/app/output/validation_stats.json")
...
def write_stats(tested: int, passed: int, from_proxxy: int, from_extras: int) -> None:
    """Best-effort sidecar with the run's counts; never fails the run."""
    try:
        with open(STATS_FILE_PATH, "w") as f:
            json.dump(
                {"tested": tested, "passed": passed,
                 "from_proxxy": from_proxxy, "from_extras": from_extras},
                f,
            )
    except OSError as e:
        print(f"  [-] Could not write stats file {STATS_FILE_PATH}: {e}")
...
async def main():
    ...
    if not proxies:
        with open(OUTPUT_FILE_PATH, 'w') as f:
            pass
        write_stats(0, 0, len(proxies_from_file), len(extra_proxies))
    ...
    write_stats(len(proxies), len(validated), len(proxies_from_file), len(extra_proxies))
    print("=== Done ===")
